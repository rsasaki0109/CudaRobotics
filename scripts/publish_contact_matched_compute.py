#!/usr/bin/env python3
"""Publish compact, content-addressed deadline-matched contact evidence."""
...
PUBLISHED_ARTIFACTS = ("calibration", "summary", "comparisons", "report")
...
def publish(
    ...
    *,
    ...
    identifier = result_id or f"contact_matched_compute_{_result_date(manifest)}"
    if not identifier or any(
        character not in "abcdefghijklmnopqrstuvwxyz0123456789_-"
        for character in identifier
    ):
        raise ValueError(
            "result id must contain only lowercase letters, numbers, '_' or '-'"
        )
    ...
    provenance = {
        "schema_version": 1,
        "evidence_mode": "published_contact_matched_compute",

#!/usr/bin/env python3
"""Publish validated v1 Docker GPU smoke evidence as an attestation."""
...
from v1_docker_gpu_evidence import evaluate_manifest
...
def build_attestation(directory: Path) -> dict[str, Any]:
    ...
    if not gate["passed"]:
        ...
        raise ValueError(
            "published Docker GPU evidence failed: " + ", ".join(failed)
        )
    ...
    if not attestation_gate["passed"]:
        ...
        raise ValueError(
            "generated Docker v1 attestation failed: "
            + ", ".join(failed)
        )
    return attestation
...
def main() -> int:
    ...
    try:
        ...
        raise SystemExit(
            f"cannot publish Docker v1 attestation: {error}"
        ) from error
    if args.check:
        if not output.is_file() or output.read_text(encoding="utf-8") != content:
            print(f"stale Docker v1 attestation: {output}")
            return 1
    else:

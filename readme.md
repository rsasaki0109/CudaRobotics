...
| Diff-MPPI, 7-DOF manipulator | On `7dof_dynamic_avoid` with `K=256`, `feedback_mppi_ref` reaches `success=1.00` at `0.090` final distance (`3.5 ms`), while vanilla `mppi` reaches `success=0.75` (`0.4 ms`). A `feedback_mppi_faithful` two-rate variant with current-action-only gains fails even at `K=8192`, confirming the need for per-step replanning in dynamic-obstacle tasks. |
...
This follow-up adds a Panda-like 7-DOF serial-arm benchmark with 14-dimensional state (7 joint angles + 7 velocities), 7-dimensional torque control, 3D workspace obstacles, and analytical dynamics Jacobians. It includes two scenarios: `7dof_shelf_reach` (static obstacle avoidance while reaching) and `7dof_dynamic_avoid` (reaching while avoiding a moving 3D obstacle). On `7dof_dynamic_avoid` at `K=256`, `feedback_mppi_ref` reaches `success=1.00` at `final_distance=0.090` using `3.5 ms` per step, while vanilla `mppi` reaches `success=0.75` at `0.273` using `0.4 ms`. An additional `feedback_mppi_faithful` variant combining the released current-action gain computation with a two-rate controller architecture (replan every other step) was tested on the base dynamic navigation suite and found to fail even at `K=8192` and `2.1 ms` per step, confirming that current-action-only feedback gains lose temporal coverage between replans. The current note is in `paper/diff_mppi_7dof_followup.md`.

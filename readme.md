...
This follow-up adds a higher-order mobile-navigation benchmark with steering lag, drag, static obstacles, and moving obstacles. The default sweep focuses on the low-budget regime `K={32,64,128,256}`, where transfer is most informative. The current fixed-budget result is intentionally narrow: `diff_mppi_3` improves the `dynbike_crossing` task at `K=32` and `K=64`, and on `dynbike_slalom` it lifts success from `0.75` to `1.00` at `K=32` while lowering final distance from `12.60` to `2.24`. The exact-time follow-up now also tunes all planners at shared `1.80 ms` and `2.00 ms` targets. In that compute-matched setting, the gains become modest but remain competitive: for example, at `1.80 ms` on `dynbike_slalom`, tuned `mppi` uses `K=13982` for `2.25` final distance, while tuned `diff_mppi_3` uses `K=589` for `2.22`. This should be read as a stronger mobile-dynamics pilot rather than as a closed high-fidelity evaluation gap. The current write-up is in `paper/diff_mppi_dynamic_bicycle_followup.md`.

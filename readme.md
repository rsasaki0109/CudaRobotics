...
| ESDF-MPPI | `esdf_mppi` | Composes the JFA ESDF (`comparison_esdf`) with MPPI: JFA builds an exact Euclidean signed distance field on the GPU (400x400 grid, 2.87 ms after warm-up), then each MPPI rollout uses a single bilinear ESDF lookup as the obstacle clearance cost (K=4096 samples x T=30 horizon, 0.335 ms / iter). No MLP training required, exact on the grid, drop-in clearance term for any sampling-based planner. |
...
| **3D ESDF (Jump Flooding 3D)** | `comparison_esdf_3d` | **3D Euclidean distance transform via 3D JFA (26-neighbour propagation). CPU brute force 32x32x16 voxels vs CUDA JFA-3D 128x128x64 voxels (64x larger field); per-voxel throughput 86,613x faster than CPU (GPU 0.854 ms / 1.05M voxels vs CPU 1156 ms / 16K voxels). JFA validation max |CPU-GPU| = 0.09 m on 32x32x16 (~0.18 cells, JFA's known small approximation error).** |

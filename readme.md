...
| **GPU batched iLQR trajectory optimization (4096 start/goal queries, unicycle + soft obstacles, gradient-based; 88.5% same local optimum, mean cost within 1.1%, 140x vs CPU)** | **GPU wavefront planner (384x384 grid, Bellman-Ford cost-to-go relaxation, one thread/cell, CPU/GPU bit-identical field + identical path, 195x vs CPU)** |
| <img src="https://rsasaki0109.github.io/CudaRobotics/gpu_batched_ilqr.gif" width="400"/> | <img src="https://rsasaki0109.github.io/CudaRobotics/gpu_wavefront_planner.gif" width="400"/> |
...
| **GPU Semi-Global Matching stereo (Hirschmuller, 384x256 x D=64, census 5x5, 4-path aggregation, one thread/scanline, CPU/GPU bit-identical disparity, ~84% within 1px vs ground truth, 46x vs CPU)** | **GPU dense direct (photometric) visual odometry (512x512 SE(2) Gauss-Newton, one thread/pixel + block reduction, CPU/GPU pose agree to ~1e-11, ground-truth recovered to 0.0002 px, 27x vs CPU)** |
| <img src="https://rsasaki0109.github.io/CudaRobotics/gpu_sgm_stereo.gif" width="400"/> | <img src="https://rsasaki0109.github.io/CudaRobotics/gpu_direct_vo.gif" width="400"/> |

...
a second distinct physical GPU model remains a declared submission blocker;
the present artifact is therefore a candidate manuscript rather than a claim
of completed multi-GPU validation.
\end{abstract}
...
\texttt{multi\_gpu\_reproduction} claim and the systems ledger remain partial
until a second distinct physical model passes the same release profile.
Real-robot closed-loop navigation is also outside the current claim boundary.
These limitations are release gates, not implied completed results.
...
anonymous source and PDF are reproducible now; the submission artifact remains
fail-closed until strict cross-model GPU evidence is complete.

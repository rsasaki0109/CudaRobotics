...
result corroborate the effect. The contribution is thus a clearly scoped,
...
gradient steps is direct evidence that the autodiff refinement, not the sampling,
is the active ingredient in the contact wins: holding the sampler fixed and
adding gradient steps strictly improves the executed pose. The boundary is
...
% NOTE: figures need regeneration for the contact-rich framing.
% Planned: (1) box-pose pushing filmstrips (mppi stuck vs diff_mppi_5 aligned),
% (2) success / angular-residual vs gradient-step-count plot (monotonicity),
% (3) CDF-MPPI vs Diff-MPPI compute-quality scatter on the smooth 7-DOF task.
...
through a higher-fidelity contact model is the natural next step. The contact

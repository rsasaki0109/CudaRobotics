...
A later dynamic-obstacle suite, exact matched-time tuning, gradient-only ablation, strengthened in-repo feedback-baseline follow-up, trace-based mechanism analysis, a pilot CartPole outside-domain benchmark, and a dynamic-bicycle mobile-navigation follow-up are recorded separately in `paper/diff_mppi_novelty_followup.md`, `paper/diff_mppi_cartpole_followup.md`, and `paper/diff_mppi_dynamic_bicycle_followup.md`.

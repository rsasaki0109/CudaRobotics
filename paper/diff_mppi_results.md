...
The later submission-oriented condensation of that story is in `paper/diff_mppi_submission_draft.md`.
...
A later dynamic-obstacle suite, exact matched-time tuning, gradient-only ablation, strengthened in-repo feedback-baseline follow-up, trace-based mechanism analysis, an uncertainty follow-up with nominal-vs-actual obstacle mismatch, a pilot CartPole outside-domain benchmark, a dynamic-bicycle mobile-navigation follow-up with its own exact-time tuning, a planar-manipulator obstacle-avoidance pilot, and a small MuJoCo standardization follow-up are recorded separately in `paper/diff_mppi_novelty_followup.md`, `paper/diff_mppi_uncertainty_followup.md`, `paper/diff_mppi_cartpole_followup.md`, `paper/diff_mppi_dynamic_bicycle_followup.md`, `paper/diff_mppi_manipulator_followup.md`, and `paper/diff_mppi_mujoco_followup.md`.
...
- Exact matched-time tuning now exists in the dynamic follow-up, but not yet across the full four-task base suite.
...
3. Extend the exact matched-time tuning workflow beyond the dynamic follow-up into the broader task suite.

// gpu_iterative_game_graph_mppi.cu
//
// GPU iterative graph-neural best-response MPPI game planner.
//
...
// evaluates a raw best-response pass, then runs damped fictitious-play updates
// against the mixed strategy profile to reduce abrupt, reach-killing reactions.
//
// Output: gif/gpu_iterative_game_graph_mppi.gif
...
constexpr float DAMPING_ALPHA = 0.52f;
...
static void mix_paths(const std::vector<float>& previous,
                      const std::vector<float>& response,
                      float alpha,
                      std::vector<float>& mixed) {
    mixed.resize(previous.size());
    float beta = 1.0f - alpha;
    for (size_t i = 0; i < previous.size(); i++) {
        mixed[i] = beta * previous[i] + alpha * response[i];
    }
}

static IterationStats compute_iteration_stats(const std::vector<float>& previous_x,
                                              const std::vector<float>& previous_y,
                                              const std::vector<float>& mixed_x,
                                              const std::vector<float>& mixed_y,
    ...
    stats.mean_unilateral_gain = game_stats.mean_unilateral_gain;
    stats.normalized_gain = game_stats.normalized_gain;
    return stats;
}
...
static cv::Mat draw_frame(const std::vector<RobotSpec>& robots,
    ...
    std::snprintf(buf, sizeof(buf),
                  "GPU iterative game graph MPPI  %d robots x %d rollouts x H=%d x %d passes  alpha=%.2f  gpu=%.2f ms  %.1fx",
                  N_ROBOTS, ROLLOUTS_PER_ROBOT, HORIZON, N_GAME_PASSES,
                  DAMPING_ALPHA, gpu_ms, speedup);
    ...
    cv::putText(img, "raw best response", cv::Point(HALF_W + 14, HEADER_H + 24),
                cv::FONT_HERSHEY_SIMPLEX, 0.50, cv::Scalar(245, 245, 245), 1, cv::LINE_AA);
    cv::putText(img, "damped fictitious play", cv::Point(2 * HALF_W + 14, HEADER_H + 24),
                cv::FONT_HERSHEY_SIMPLEX, 0.50, cv::Scalar(245, 245, 245), 1, cv::LINE_AA);
    std::snprintf(buf, sizeof(buf),
                  "collisions %d -> %d -> %d (-%.1f%% raw, -%.1f%% iter)  CVaR %.2f -> %.2f (-%.1f%%)",
    ...
    std::snprintf(buf, sizeof(buf),
                  "reach %d/%d -> %d/%d -> %d/%d   residual %.2f%% -> %.2f%%   path delta %.3f   intent top1 %.1f%%",
                  one_shot.reached, N_ROBOTS, raw_response.reached, N_ROBOTS,
                  iterative.reached, N_ROBOTS, raw_game.normalized_gain,
                  final_game.normalized_gain, last_iter.mean_path_delta,
...
int main() {
    ...
    for (int pass = 1; pass < N_GAME_PASSES; pass++) {
        ...
        mix_paths(cpu_peer_x, cpu_response_x, DAMPING_ALPHA, cpu_mixed_x);
        mix_paths(cpu_peer_y, cpu_response_y, DAMPING_ALPHA, cpu_mixed_y);
    ...
    for (int pass = 1; pass < N_GAME_PASSES; pass++) {
        ...
        mix_paths(peer_x, response_x, DAMPING_ALPHA, mixed_x);
        mix_paths(peer_y, response_y, DAMPING_ALPHA, mixed_y);
        iter_stats.push_back(compute_iteration_stats(peer_x, peer_y, mixed_x, mixed_y,
                                                     pass_game));
        peer_x.swap(mixed_x);
        peer_y.swap(mixed_y);
    ...
    std::printf("CPU iterative game MPPI: %.3f ms (%d robots x %d rollouts x H=%d x %d passes; pass ms %.3f / %.3f / %.3f / %.3f)\n",
                cpu_ms, N_ROBOTS, ROLLOUTS_PER_ROBOT, HORIZON, N_GAME_PASSES,
                cpu_pass_ms[0], cpu_pass_ms[1], cpu_pass_ms[2], cpu_pass_ms[3]);
    std::printf("GPU iterative game graph MPPI: %.3f ms (one-shot+3 damped best-response batches, %.1fx vs CPU equivalent rollout eval; pass ms %.3f / %.3f / %.3f / %.3f)\n",
    ...
    std::printf("Damped iterative team: collisions %d, reached %d/%d, deadlocks %d, min sep %.3f, terminal avg %.3f, social risk avg/max %.3f/%.3f, collision CVaR %.3f, final residual %.3f (%.2f%%, max %.3f), collision reduction %.1f%%, risk reduction %.1f%%, CVaR reduction %.1f%%, separation gain %.3f\n",
    ...
    for (size_t i = 0; i < iter_stats.size(); i++) {
        ...
        std::printf("Pass %zu damped update: mean path delta %.3f, max path delta %.3f, unilateral residual %.3f (%.2f%%)\n",
                    i + 1, s.mean_path_delta, s.max_path_delta,
                    s.mean_unilateral_gain, s.normalized_gain);
    }

    std::system("mkdir -p gif");
    cv::VideoWriter video("gif/gpu_iterative_game_graph_mppi.avi",
                          cv::VideoWriter::fourcc('M', 'J', 'P', 'G'),
                          VIDEO_FPS, cv::Size(PANEL_W, PANEL_H));
    if (!video.isOpened()) {
        std::fprintf(stderr, "failed to open gif/gpu_iterative_game_graph_mppi.avi\n");
        return 1;
    }
    ...
    avi_to_gif("gif/gpu_iterative_game_graph_mppi.avi",
               "gif/gpu_iterative_game_graph_mppi.gif", 8, 720);
    std::printf("GIF saved to gif/gpu_iterative_game_graph_mppi.gif\n");

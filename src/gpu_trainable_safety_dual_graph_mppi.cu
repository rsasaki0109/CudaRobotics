// gpu_trainable_safety_dual_graph_mppi.cu
//
// GPU trainable safety-dual prior graph-neural MPPI game planner.
//
...
// trainable-prior safety-dual update.  The prior is a tiny CPU-trained MLP
// distilled from synthetic graph-risk labels: it predicts per-robot safety
// duals, alpha corrections, and a scale hint, while the final CVaR/reach check
// remains an explicit planner guard.
//
// Output: gif/gpu_trainable_safety_dual_graph_mppi.gif
...
static cv::Mat draw_frame(const std::vector<RobotSpec>& robots,
    ...
    std::snprintf(buf, sizeof(buf),
                  "GPU trainable safety-dual prior graph MPPI  %d robots x %d rollouts x H=%d x %d passes  alpha %.2f..%.2f  gpu=%.2f ms  %.1fx",
    ...
    cv::putText(img, "regret-matched no-regret", cv::Point(2 * HALF_W + 14, HEADER_H + 24),
                cv::FONT_HERSHEY_SIMPLEX, 0.50, cv::Scalar(245, 245, 245), 1, cv::LINE_AA);
    cv::putText(img, "trainable safety-dual prior", cv::Point(3 * HALF_W + 14, HEADER_H + 24),
                cv::FONT_HERSHEY_SIMPLEX, 0.50, cv::Scalar(245, 245, 245), 1, cv::LINE_AA);
    std::snprintf(buf, sizeof(buf),
...
int main() {
    ...
    for (int pass = 1; pass < N_GAME_PASSES; pass++) {
        ...
        mix_paths_trainable_safety_prior(prior_net, robots, cpu_safe_peer_x,
                                         cpu_safe_peer_y, cpu_safe_response_x,
                                         cpu_safe_response_y, cpu_safe_response,
                                         cpu_safe_previous_selected, cpu_safe_selected,
                                         pass, cpu_safe_mixed_x, cpu_safe_mixed_y,
                                         cpu_safe_stats);
    ...
    for (int pass = 1; pass < N_GAME_PASSES; pass++) {
        ...
        mix_paths_trainable_safety_prior(prior_net, robots, safe_peer_x, safe_peer_y,
    ...
    std::printf("CPU trainable safety-dual prior game MPPI: %.3f ms (%d robots x %d rollouts x H=%d x %zu rollout batches)\n",
                cpu_ms, N_ROBOTS, ROLLOUTS_PER_ROBOT, HORIZON, cpu_pass_ms.size());
    std::printf("GPU trainable safety-dual prior game graph MPPI: %.3f ms (one-shot+shared raw+2 no-regret+2 trainable-prior best-response batches, %.1fx vs CPU equivalent rollout eval; %zu GPU batches)\n",
    ...
    std::printf("No-regret team: collisions %d, reached %d/%d, deadlocks %d, min sep %.3f, terminal avg %.3f, social risk avg/max %.3f/%.3f, collision CVaR %.3f, final residual %.3f (%.2f%%, max %.3f), collision reduction %.1f%%, risk reduction %.1f%%, CVaR reduction %.1f%%, separation gain %.3f\n",
                ...
                noregret_sep_gain);
    std::printf("Trainable-prior safe team: collisions %d, reached %d/%d, deadlocks %d, min sep %.3f, terminal avg %.3f, social risk avg/max %.3f/%.3f, collision CVaR %.3f, final residual %.3f (%.2f%%, max %.3f), collision reduction %.1f%%, risk reduction %.1f%%, CVaR reduction %.1f%%, separation gain %.3f\n",
    ...
    cv::VideoWriter video("gif/gpu_trainable_safety_dual_graph_mppi.avi",
                          cv::VideoWriter::fourcc('M', 'J', 'P', 'G'),
                          VIDEO_FPS, cv::Size(PANEL_W, PANEL_H));
    if (!video.isOpened()) {
        std::fprintf(stderr, "failed to open gif/gpu_trainable_safety_dual_graph_mppi.avi\n");
        return 1;
    }
    ...
    avi_to_gif("gif/gpu_trainable_safety_dual_graph_mppi.avi",
               "gif/gpu_trainable_safety_dual_graph_mppi.gif", 8, 720);
    std::printf("GIF saved to gif/gpu_trainable_safety_dual_graph_mppi.gif\n");

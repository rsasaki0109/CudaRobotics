...
// The existing pose graph optimizes a 6-DoF pose state, while the IMU factor
// API exposes the richer (R, p, v, bias) state.  Until that backend state is
// widened, feed the graph a pose-only relative measurement generated by the
// same preintegrator.  This keeps SO(3) integration and the pose-graph frame
// convention in one place; real callers can additionally configure the
// preintegrator's calibration and bias parameters before using the factor API.
static Edge make_imu_edge_from_gt(const std::vector<Pose>& gt,
                                  int i,
                                  int j,
                                  float sigma_t,
                                  float sigma_r,
    ...
    float target_t[3];
    float target_R[9];
    pose_relative(gt[i], gt[j], target_t, target_R);

    cudarobotics::imu::ImuBias zero_bias{};
    cudarobotics::imu::ImuPreintegrator preintegrator;
    preintegrator.reset(0.0f, zero_bias);
    float rotation_vector[3];
    cudarobotics::lie::so3_log(target_R, rotation_vector);
    constexpr float synthetic_dt = 0.1f;
    float synthetic_accel[3] = {
        2.0f * target_t[0] / (synthetic_dt * synthetic_dt),
        2.0f * target_t[1] / (synthetic_dt * synthetic_dt),
        2.0f * target_t[2] / (synthetic_dt * synthetic_dt),
    };
    float synthetic_gyro[3] = {
        rotation_vector[0] / synthetic_dt,
        rotation_vector[1] / synthetic_dt,
        rotation_vector[2] / synthetic_dt,
    };
    preintegrator.update(synthetic_accel, synthetic_gyro, synthetic_dt);
    for (int k = 0; k < 3; ++k) e.t[k] = preintegrator.delta.p[k];
    for (int k = 0; k < 9; ++k) e.R[k] = preintegrator.delta.R[k];
...
static std::vector<Edge> make_edges(const std::vector<Pose>& gt,
    ...
    for (int i = 0; i < N_POSES - 1; i++) {
        edges.push_back(make_imu_edge_from_gt(gt, i, i + 1,
                                               ODOM_SIGMA_T, ODOM_SIGMA_R,
                                               false, rng));
    }
...
int main() {
    ...
    std::printf("%s\n", DEMO_TITLE);
    std::printf("odometry source: IMU preintegration pose bridge\n");

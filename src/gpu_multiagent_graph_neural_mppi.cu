// gpu_multiagent_graph_neural_mppi.cu
//
// GPU multi-agent graph-neural MPPI planner.
//
// A batch of robots crosses a shared interaction zone.  The baseline evaluates
// independent kinodynamic MPPI rollouts for every robot.  The coordinated
// variant uses a lightweight interaction graph over nominal peer trajectories:
// each rollout receives graph messages from nearby robots, biases its local
// control target away from predicted conflicts, and includes pairwise social
// risk in the selection objective.
//
// Output: gif/gpu_multiagent_graph_neural_mppi.gif
...
__host__ __device__ static inline void rollout_step(const RobotSpec* robots,
    ...
    float priority_phase = coordinated ? 0.08f * (r.priority - 0.62f) : 0.0f;
    ...
    float social_slow = coordinated ? clampf(social_risk / 3.4f, 0.0f, 1.0f) : 0.0f;
    ...
    float speed_sigma = coordinated ? 0.17f : 0.28f;
    float steer_sigma = coordinated ? 0.17f : 0.28f;
...
__host__ __device__ static inline RolloutResult evaluate_rollout(const RobotSpec* robots,
    ...
    for (int k = 0; k < HORIZON; k++) {
        ...
        float smooth;
        float terrain;
        rollout_step(robots, robot_id, rollout, k, coordinated, s, prev_speed, prev_steer,
        ...
        float close_penalty = step_min_sep < COLLISION_MARGIN
                            ? 82.0f * sqr(COLLISION_MARGIN - step_min_sep)
                            : 0.0f;
        float social_step = 22.0f * social_risk + close_penalty;
    ...
    RolloutResult out;
    out.select_cost = coordinated ? full_cost : blind_cost;
...
__global__ void rollout_kernel(const RobotSpec* __restrict__ robots,
    ...
    results[idx] = evaluate_rollout(robots, robot_id, rollout, coordinated);

    if (!coordinated || robot_id >= SAMPLE_ROBOTS || rollout >= SAMPLE_ROLLOUTS) return;
    ...
    constexpr int METRIC_START_STEP = 8;
    for (int k = METRIC_START_STEP; k < HORIZON; k++) {
        float social_risk;
        float min_sep;
        float route_error;
        float smooth;
...
static cv::Mat draw_frame(const std::vector<RobotSpec>& robots,
    ...
    std::snprintf(buf, sizeof(buf),
                  "GPU multi-agent graph-neural MPPI  %d robots x %d rollouts x H=%d  gpu=%.2f ms  %.1fx",
    ...
    cv::putText(img, "independent MPPI + conflict graph", cv::Point(14, HEADER_H + 24),
                cv::FONT_HERSHEY_SIMPLEX, 0.50, cv::Scalar(245, 245, 245), 1, cv::LINE_AA);
    cv::putText(img, "coordinated graph-neural MPPI", cv::Point(HALF_W + 14, HEADER_H + 24),
                cv::FONT_HERSHEY_SIMPLEX, 0.50, cv::Scalar(245, 245, 245), 1, cv::LINE_AA);
    std::snprintf(buf, sizeof(buf),
...
int main() {
    ...
    std::printf("CPU independent+coordinated multi-agent MPPI: %.3f ms (%d robots x %d rollouts x H=%d x 2 modes; coordinated %.3f ms)\n",
                cpu_ms, N_ROBOTS, ROLLOUTS_PER_ROBOT, HORIZON, cpu_coordinated_ms);
    std::printf("GPU multi-agent graph-neural MPPI: %.3f ms (independent+coordinated batches, %.1fx vs CPU equivalent rollout eval)\n",
                gpu_ms, speedup);
    std::printf("Independent team: collisions %d, reached %d/%d, deadlocks %d, min sep %.3f, terminal avg %.3f, social risk avg/max %.3f/%.3f\n",
    ...
    std::printf("Coordinated team: collisions %d, reached %d/%d, deadlocks %d, min sep %.3f, terminal avg %.3f, social risk avg/max %.3f/%.3f, collision reduction %.1f%%, risk reduction %.1f%%, separation gain %.3f\n",
    ...
    if (!video.isOpened()) {
        std::fprintf(stderr, "failed to open gif/gpu_multiagent_graph_neural_mppi.avi\n");
        return 1;
    }
    ...
    avi_to_gif("gif/gpu_multiagent_graph_neural_mppi.avi",
               "gif/gpu_multiagent_graph_neural_mppi.gif", 8, 560);
    std::printf("GIF saved to gif/gpu_multiagent_graph_neural_mppi.gif\n");

// gpu_kinodynamic_graph_neural_mppi.cu
//
// GPU kinodynamic graph-neural MPPI planner.
//
// A learned experience-graph route supplies a sparse global prior to an
// Ackermann-like nonholonomic MPPI rollout optimizer.  Each CUDA thread
// evaluates one stochastic speed/steering sequence under learned traversability,
// dynamic obstacle risk, terminal goal error, heading alignment, and route
// tracking cost.  The demo compares graph-neural guided MPPI against a
// goal-only kinodynamic MPPI baseline on the same scene.
//
// Output: gif/gpu_kinodynamic_graph_neural_mppi.gif
...
__host__ __device__ static inline void learned_control_prior(const Pose2& s,
    ...
    float route_boost = guided ? expv : 0.0f;
    ...
    if (guided) {
        speed = clampf(0.52f + 1.18f * fast_sigmoid(1.75f * latent0 + 0.34f * dist
                       ...
                       MIN_SPEED, MAX_SPEED);
        steer = clampf(0.58f * fast_tanh(1.70f * heading_error + 0.42f * latent1),
                       -MAX_STEER, MAX_STEER);
    } else {
        speed = clampf(0.28f + 0.82f * fast_sigmoid(0.60f * align + 0.13f * dist
                                                    - 1.05f * rough_slow - 0.82f * risk_slow),
                       MIN_SPEED, MAX_SPEED);
        steer = clampf(0.25f * fast_tanh(0.88f * heading_error + 0.16f * latent1),
                       -MAX_STEER, MAX_STEER);
    }
}
...
__host__ __device__ static inline void rollout_step(int rollout,
    ...
    if (guided) {
        ...
        float lookahead = progress + 0.20f + 0.04f * hash_unit(rollout, k / 8, 8);
    ...
    learned_control_prior(s, tx, ty, expv, terrain, risk, guided,
    ...
    float speed_sigma = guided ? 0.18f : 0.25f;
    float steer_sigma = guided ? 0.14f : 0.22f;
...
__host__ __device__ static inline RolloutResult evaluate_rollout(int rollout,
    ...
    for (int k = 0; k < HORIZON; k++) {
        float route_d, terrain, risk, heading_error, steer, smooth;
        rollout_step(rollout, k, guided, rx, ry, cost_field, risk_field, route_field, exp_field,
                     s, prev_speed, prev_steer, route_d, terrain, risk,
        ...
        cost += 0.58f * terrain + 12.0f * risk + collision
    ...
    cost += 38.0f * terminal * terminal + 4.0f * route_terminal
...
static std::vector<cv::Point2f> reconstruct_trajectory(int rollout,
    ...
    for (int k = 0; k < HORIZON; k++) {
        ...
        rollout_step(rollout, k, guided, rx.data(), ry.data(), cost_field.data(), risk_field.data(),
                     route_field.data(), exp_field.data(),
                     s, prev_speed, prev_steer, route_d, terrain, risk,
                     heading_error, steer, smooth);
...
static void draw_legend(cv::Mat& img) {
    ...
    cv::putText(img, "steering rollouts", cv::Point(x0 + 44, y0 + 53),
                cv::FONT_HERSHEY_SIMPLEX, 0.38, cv::Scalar(235, 235, 235), 1, cv::LINE_AA);
...
static cv::Mat draw_frame(const std::vector<cv::Point2f>& route,
    ...
    std::snprintf(buf, sizeof(buf),
                  "GPU kinodynamic graph-neural MPPI  %d rollouts x H=%d  gpu=%.2f ms  cpu=%.0f ms  %.1fx",
    ...
    cv::putText(img, "experience graph + learned route prior", cv::Point(14, HEADER_H + 24),
                cv::FONT_HERSHEY_SIMPLEX, 0.52, cv::Scalar(245, 245, 245), 1, cv::LINE_AA);
    cv::putText(img, "nonholonomic steering rollouts", cv::Point(HALF_W + 14, HEADER_H + 24),
                cv::FONT_HERSHEY_SIMPLEX, 0.52, cv::Scalar(245, 245, 245), 1, cv::LINE_AA);
    std::snprintf(buf, sizeof(buf),
...
int main() {
    ...
    float risk_delta = 100.0f * (guided_best.mean_risk / fmaxf(unguided_best.mean_risk, 1.0e-6f) - 1.0f);
    std::printf("CPU unguided+guided kinodynamic MPPI: %.3f ms (%d rollouts x H=%d x 2 batches; guided %.3f ms)\n",
                cpu_ms, ROLLOUTS, HORIZON, cpu_guided_ms);
    std::printf("GPU kinodynamic graph-neural MPPI: %.3f ms (guided+unguided batches, %.1fx vs CPU equivalent rollout eval)\n",
                gpu_ms, speedup);
    ...
    cv::VideoWriter video("gif/gpu_kinodynamic_graph_neural_mppi.avi",
                          cv::VideoWriter::fourcc('M', 'J', 'P', 'G'),
                          VIDEO_FPS, cv::Size(PANEL_W, PANEL_H));
    if (!video.isOpened()) {
        std::fprintf(stderr, "failed to open gif/gpu_kinodynamic_graph_neural_mppi.avi\n");
        return 1;
    }
    ...
    avi_to_gif("gif/gpu_kinodynamic_graph_neural_mppi.avi",
               "gif/gpu_kinodynamic_graph_neural_mppi.gif", 8, 560);
    std::printf("GIF saved to gif/gpu_kinodynamic_graph_neural_mppi.gif\n");

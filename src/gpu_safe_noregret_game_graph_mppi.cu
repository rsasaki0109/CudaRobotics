// gpu_safe_noregret_game_graph_mppi.cu
//
// GPU safety-constrained no-regret graph-neural MPPI game planner.
//
...
// evaluates a raw best-response pass, then runs both vanilla and
// safety-constrained regret-matching updates.  The constrained variant treats
// collision-tail CVaR as a budgeted dual term, boosting alpha when a response
// relieves tail risk and clipping it when the response worsens separation.
//
// Output: gif/gpu_safe_noregret_game_graph_mppi.gif
...
static void mix_paths_safety_constrained(const std::vector<RobotSpec>& robots,
    ...
    for (int robot = 0; robot < N_ROBOTS; robot++) {
        ...
        float safety_dual = 1.0f
                          + 0.46f * fmaxf(tail_relief, 0.0f)
                          + 0.24f * fmaxf(sep_relief, 0.0f)
                          - 0.34f * tail_violation
                          - 0.36f * sep_violation
                          - 0.12f * terminal_violation;
        safety_dual = clampf(safety_dual, 0.50f, 1.48f);

        alpha_base[robot] = clampf(alpha * safety_dual, SAFETY_MIN_ALPHA, SAFETY_MAX_ALPHA);
    ...
    const float scales[] = {0.58f, 0.72f, 0.86f, 1.00f, 1.14f,
                            1.30f, 1.46f, 1.64f, 1.82f};
    ...
    for (float scale : scales) {
        ...
        float score = 3.0f * cvar_over
                    + 0.28f * collision_over
                    + 24.0f * reach_loss
                    + 18.0f * static_cast<float>(metrics.deadlocks)
                    + 8.0f * terminal_over
                    + 3.0f * sep_over
                    + 0.65f * previous_cvar_over
                    + 0.08f * static_cast<float>(metrics.collisions)
                    + 0.18f * metrics.collision_cvar;
        if (score < best_score) {
            best_score = score;
            best_scale = scale;
...
static cv::Mat draw_frame(const std::vector<RobotSpec>& robots,
    ...
    std::snprintf(buf, sizeof(buf),
                  "GPU safety-constrained no-regret graph MPPI  %d robots x %d rollouts x H=%d x %d passes  safe alpha %.2f..%.2f  gpu=%.2f ms  %.1fx",
    ...
    cv::putText(img, "regret-matched no-regret", cv::Point(2 * HALF_W + 14, HEADER_H + 24),
                cv::FONT_HERSHEY_SIMPLEX, 0.50, cv::Scalar(245, 245, 245), 1, cv::LINE_AA);
    cv::putText(img, "safe no-regret", cv::Point(3 * HALF_W + 14, HEADER_H + 24),
                cv::FONT_HERSHEY_SIMPLEX, 0.50, cv::Scalar(245, 245, 245), 1, cv::LINE_AA);
    std::snprintf(buf, sizeof(buf),
...
int main() {
    ...
    for (int pass = 1; pass < N_GAME_PASSES; pass++) {
        ...
        mix_paths_safety_constrained(robots, cpu_safe_peer_x, cpu_safe_peer_y,
                                     cpu_safe_response_x, cpu_safe_response_y,
    ...
    for (int pass = 1; pass < N_GAME_PASSES; pass++) {
        ...
        mix_paths_safety_constrained(robots, safe_peer_x, safe_peer_y,
    ...
    std::printf("CPU safety-constrained no-regret game MPPI: %.3f ms (%d robots x %d rollouts x H=%d x %zu rollout batches)\n",
                cpu_ms, N_ROBOTS, ROLLOUTS_PER_ROBOT, HORIZON, cpu_pass_ms.size());
    std::printf("GPU safety-constrained no-regret game graph MPPI: %.3f ms (one-shot+shared raw+2 no-regret+2 safe best-response batches, %.1fx vs CPU equivalent rollout eval; %zu GPU batches)\n",
    ...
    std::printf("No-regret team: collisions %d, reached %d/%d, deadlocks %d, min sep %.3f, terminal avg %.3f, social risk avg/max %.3f/%.3f, collision CVaR %.3f, final residual %.3f (%.2f%%, max %.3f), collision reduction %.1f%%, risk reduction %.1f%%, CVaR reduction %.1f%%, separation gain %.3f\n",
                ...
                noregret_sep_gain);
    std::printf("Safe no-regret team: collisions %d, reached %d/%d, deadlocks %d, min sep %.3f, terminal avg %.3f, social risk avg/max %.3f/%.3f, collision CVaR %.3f, final residual %.3f (%.2f%%, max %.3f), collision reduction %.1f%%, risk reduction %.1f%%, CVaR reduction %.1f%%, separation gain %.3f\n",
    ...
    for (size_t i = 0; i < safe_stats.size(); i++) {
        ...
        std::printf("Pass %zu safe no-regret update: mean path delta %.3f, max path delta %.3f, alpha avg/range %.3f [%.3f, %.3f], safety dual avg/max %.3f/%.3f, violation avg/max %.3f/%.3f, CVaR %.3f -> %.3f, scale %.2f, residual %.3f (%.2f%%)\n",
                    i + 1, s.mean_path_delta, s.max_path_delta,
                    s.mean_alpha, s.min_alpha, s.max_alpha,
    ...
    if (!video.isOpened()) {
        std::fprintf(stderr, "failed to open gif/gpu_safe_noregret_game_graph_mppi.avi\n");
        return 1;
    }
    ...
    avi_to_gif("gif/gpu_safe_noregret_game_graph_mppi.avi",
               "gif/gpu_safe_noregret_game_graph_mppi.gif", 8, 720);
    std::printf("GIF saved to gif/gpu_safe_noregret_game_graph_mppi.gif\n");

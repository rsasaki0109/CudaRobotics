// gpu_spatiotemporal_neural_astar_traversability.cu
//
// Batched GPU spatiotemporal neural A* on learned traversability cost.
//
// A fixed-weight MLP-style heuristic predicts terrain-aware cost-to-go from
// roughness, clearance, height, route prior, goal geometry, and dynamic
// obstacle time-to-collision risk.  Each query gets one CUDA block that
// parallelizes the open-set min-f reduction over a 192x128 grid.  Transition
// costs include moving-obstacle risk at the estimated cell arrival time, so
// the planner bends around dynamic conflicts instead of only static terrain.
//
// Output: gif/gpu_spatiotemporal_neural_astar_traversability.gif
...
constexpr float HEURISTIC_WEIGHT = 4.80f;
constexpr float DYNAMIC_COST_WEIGHT = 7.40f;
constexpr float DYNAMIC_HEURISTIC_WEIGHT = 19.0f;
constexpr float TIME_SCALE = 0.035f;
constexpr float MAX_TIME = 22.0f;
constexpr int MOVING_OBSTACLES = 4;
...
__host__ __device__ static inline float world_x_from_ix(int ix) {
    return (static_cast<float>(ix) + 0.5f) / GRID_W * WORLD_W;
}

__host__ __device__ static inline float world_y_from_iy(int iy) {
    return (static_cast<float>(iy) + 0.5f) / GRID_H * WORLD_H;
}

__host__ __device__ static inline float arrival_time_from_cost(float g_cost) {
    return clampf(g_cost * TIME_SCALE, 0.0f, MAX_TIME);
}

__host__ __device__ static inline float query_start_time(int query) {
    return 0.38f * static_cast<float>(query % 8);
}
...
__host__ __device__ static inline float dynamic_obstacle_risk(float x, float y, float t) {
    float risk = 0.0f;
    for (int k = 0; k < MOVING_OBSTACLES; k++) {
        float ox, oy, radius;
        moving_obstacle(k, t, ox, oy, radius);
        float margin = sqrtf(sqr(x - ox) + sqr(y - oy)) - radius;
        float near_risk = expf(-0.5f * sqr(margin / 0.52f));
        float collision_risk = sigmoid((0.18f - margin) * 8.0f);
        risk += 0.70f * near_risk + 1.35f * collision_risk;
    }
    return clampf(risk, 0.0f, 4.0f);
}

__host__ __device__ static inline float dynamic_heuristic_bias(float x, float y) {
    float route_x = GOAL_X - START_X;
    float route_y = GOAL_Y - START_Y;
    float route_len2 = route_x * route_x + route_y * route_y;
    float progress = clampf(((x - START_X) * route_x + (y - START_Y) * route_y) / route_len2,
                            0.0f, 1.0f);
    float route_len = sqrtf(route_len2);
    float eta = progress * route_len * TIME_SCALE * 31.0f;
    float risk = dynamic_obstacle_risk(x, y, eta);
    float future_risk = dynamic_obstacle_risk(x, y, fminf(eta + 1.25f, MAX_TIME));
    return DYNAMIC_HEURISTIC_WEIGHT * (0.72f * risk + 0.28f * future_risk);
}
...
static std::vector<Cell> make_cells() {
    ...
    for (int iy = 0; iy < GRID_H; iy++) {
        for (int ix = 0; ix < GRID_W; ix++) {
            ...
            float x = world_x_from_ix(ix);
            float y = world_y_from_iy(iy);
...
static std::vector<float> make_dynamic_heuristic(const std::vector<Cell>& cells) {
    std::vector<float> heuristic(N_CELLS);
    for (int i = 0; i < N_CELLS; i++) {
        heuristic[i] = cells[i].heuristic + dynamic_heuristic_bias(cells[i].x, cells[i].y);
    }
    return heuristic;
}
...
static Metrics evaluate_path(const std::vector<Cell>& cells,
    ...
    float dynamic_sum = 0.0f;
    float dynamic_max = 0.0f;
    for (int idx : path) {
        cost_sum += cells[idx].cost;
        blocked_sum += cells[idx].blocked ? 1.0f : 0.0f;
        float t = start_time + arrival_time_from_cost(g[idx] < INF_COST * 0.5f ? g[idx] : 0.0f);
        float risk = dynamic_obstacle_risk(cells[idx].x, cells[idx].y, t);
        dynamic_sum += risk;
        dynamic_max = std::max(dynamic_max, risk);
    }
    ...
    m.path_dynamic_risk = path.empty() ? 0.0f : dynamic_sum / static_cast<float>(path.size());
    m.max_dynamic_risk = dynamic_max;
    return m;
}
...
static SearchResult cpu_search(const std::vector<Cell>& cells,
    ...
    auto maybe_snapshot = [&]() {
        ...
        Metrics m = evaluate_path(cells, result.g, path, reached, expanded, opened,
                                  reached ? goal_idx_found : trace_idx, start_time);
    ...
    for (int iter = 0; iter < MAX_EXPANSIONS; iter++) {
        ...
        for (int a = 0; a < 8; a++) {
            ...
            float base_tentative = result.g[best_idx] + transition_cost(cells[best_idx].cost,
                                                                        cells[ni].cost,
                                                                        len);
            float tentative = base_tentative;
            if (use_dynamic) {
                float arrival_t = start_time + arrival_time_from_cost(base_tentative);
                tentative += DYNAMIC_COST_WEIGHT
                           * dynamic_obstacle_risk(cells[ni].x, cells[ni].y, arrival_t);
            }
...
static double timed_cpu_search(const std::vector<Cell>& cells,
    ...
    result = cpu_search(cells, heuristic, heuristic_weight, use_dynamic, start_time, nullptr);
...
__global__ void neural_astar_kernel(const float* __restrict__ cost,
    ...
    int base = q * N_CELLS;
    int stats_base = q * 4;
    float start_t = query_start_time(q);
    if (tid == 0) {
        done = 0;
        ...
        if (tid < 8) {
            ...
            if (nx >= 0 && nx < GRID_W && ny >= 0 && ny < GRID_H) {
                ...
                if (!blocked[ni] && !closed[ngi]) {
                    float base_tentative = g[base + current] + transition_cost(cost[current], cost[ni], len);
                    float tentative = base_tentative;
                    if (use_dynamic) {
                        float arrival_t = start_t + arrival_time_from_cost(base_tentative);
                        tentative += DYNAMIC_COST_WEIGHT
                                   * dynamic_obstacle_risk(world_x_from_ix(nx),
                                                           world_y_from_iy(ny),
                                                           arrival_t);
                    }
                    if (tentative < g[ngi]) {
...
static void draw_dynamic_obstacles(cv::Mat& img, int x0, float t) {
    for (int k = 0; k < MOVING_OBSTACLES; k++) {
        for (int trail = 5; trail >= 1; trail--) {
            float ox, oy, radius;
            moving_obstacle(k, fmaxf(0.0f, t - 0.42f * trail), ox, oy, radius);
            int ix = static_cast<int>(ox / WORLD_W * GRID_W);
            int iy = static_cast<int>(oy / WORLD_H * GRID_H);
            cv::circle(img, to_px(ix, iy, x0), 3 + trail,
                       cv::Scalar(34 + 8 * trail, 78 + 8 * trail, 148 + 13 * trail),
                       1, cv::LINE_AA);
        }
        float ox, oy, radius;
        moving_obstacle(k, t, ox, oy, radius);
        ...
        cv::circle(img, to_px(ix, iy, x0), px_radius + 8,
                   cv::Scalar(34, 72, 138), 1, cv::LINE_AA);
        cv::circle(img, to_px(ix, iy, x0), px_radius,
...
static cv::Mat draw_frame(const std::vector<Cell>& cells,
    ...
    float frame_time = start_time + (snap.metrics.reached
                       ? arrival_time_from_cost(snap.metrics.goal_cost)
                       : clampf(static_cast<float>(snap.expanded) * 0.006f, 0.0f, MAX_TIME));
    draw_dynamic_obstacles(img, 0, frame_time);
    draw_dynamic_obstacles(img, HALF_W, frame_time);
    draw_markers(img, 0);
    ...
    double cpu_batch_ms = cpu_dynamic_ms * static_cast<double>(BATCH_QUERIES);
    double speedup = gpu_ms > 0.0 ? cpu_batch_ms / gpu_ms : 0.0;
    float risk_reduction = 100.0f * (1.0f - snap.metrics.max_dynamic_risk
                                           / fmaxf(static_metrics.max_dynamic_risk, 1.0e-6f));
    char buf[256];
    std::snprintf(buf, sizeof(buf),
                  "GPU spatiotemporal neural A*  %dx%dx%d  gpu=%.2f ms  cpu_seq=%.0f ms  %.1fx  risk -%.1f%%",
                  BATCH_QUERIES, GRID_W, GRID_H, gpu_ms, cpu_batch_ms, speedup,
                  risk_reduction);
    cv::putText(img, buf, cv::Point(12, 28),
                cv::FONT_HERSHEY_SIMPLEX, 0.50, cv::Scalar(245, 245, 245), 1, cv::LINE_AA);
    cv::putText(img, "learned cost + time-to-collision risk", cv::Point(14, HEADER_H + 24),
                cv::FONT_HERSHEY_SIMPLEX, 0.52, cv::Scalar(245, 245, 245), 1, cv::LINE_AA);
    cv::putText(img, "dynamic-risk A* frontier", cv::Point(HALF_W + 14, HEADER_H + 24),
                cv::FONT_HERSHEY_SIMPLEX, 0.52, cv::Scalar(245, 245, 245), 1, cv::LINE_AA);
    std::snprintf(buf, sizeof(buf),
                  "expanded %d / dijkstra %d (-%.1f%%)  steps=%d  g=%.2f  risk avg/max=%.2f/%.2f  dijkstra=%.1f ms",
                  snap.metrics.expanded, dijkstra_metrics.expanded, reduction,
                  snap.metrics.path_steps, snap.metrics.goal_cost,
                  snap.metrics.path_dynamic_risk, snap.metrics.max_dynamic_risk, dijkstra_ms);
...
int main() {
    ...
    std::vector<float> heuristic(N_CELLS);
    std::vector<float> dynamic_heuristic = make_dynamic_heuristic(cells);
    ...
    SearchResult cpu_static;
    SearchResult cpu_dynamic;
    SearchResult cpu_dijkstra;
    double cpu_static_ms = timed_cpu_search(cells, heuristic, HEURISTIC_WEIGHT,
                                            0, 0.0f, cpu_static);
    double cpu_dynamic_ms = timed_cpu_search(cells, dynamic_heuristic, HEURISTIC_WEIGHT,
                                             1, 0.0f, cpu_dynamic);
    ...
    CUDA_CHECK(cudaMemcpy(d_heuristic, dynamic_heuristic.data(), N_CELLS * sizeof(float), cudaMemcpyHostToDevice));
    ...
    neural_astar_kernel<<<BATCH_QUERIES, THREADS>>>(d_cost, d_blocked, d_heuristic,
                                                   HEURISTIC_WEIGHT, 1, d_g, d_parent,
    ...
    int gpu_goal_idx = stats[2] ? stats[3] : best_partial_idx(gpu_g, gpu_closed, dynamic_heuristic);
    ...
    Metrics gpu_metrics = evaluate_path(cells, gpu_g, gpu_path, stats[2], stats[0],
                                        stats[1], gpu_goal_idx, 0.0f);

    std::vector<Snapshot> snapshots;
    SearchResult visual = cpu_search(cells, dynamic_heuristic, HEURISTIC_WEIGHT,
                                     1, 0.0f, &snapshots);
    ...
    float risk_reduction = 100.0f * (1.0f - gpu_metrics.max_dynamic_risk
                                           / fmaxf(cpu_static.metrics.max_dynamic_risk, 1.0e-6f));
    std::printf("CPU static neural A*: %.3f ms, expanded %d, path cost %.3f, dynamic risk avg/max %.3f/%.3f, reached %d\n",
                cpu_static_ms, cpu_static.metrics.expanded, cpu_static.metrics.goal_cost,
                cpu_static.metrics.path_dynamic_risk, cpu_static.metrics.max_dynamic_risk,
                cpu_static.metrics.reached);
    std::printf("CPU spatiotemporal neural A*: %.3f ms, expanded %d, path cost %.3f, dynamic risk avg/max %.3f/%.3f, reached %d\n",
                cpu_dynamic_ms, cpu_dynamic.metrics.expanded, cpu_dynamic.metrics.goal_cost,
                cpu_dynamic.metrics.path_dynamic_risk, cpu_dynamic.metrics.max_dynamic_risk,
                cpu_dynamic.metrics.reached);
    std::printf("CPU dynamic Dijkstra: %.3f ms, expanded %d, path cost %.3f, dynamic risk avg/max %.3f/%.3f, reached %d\n",
                dijkstra_ms, cpu_dijkstra.metrics.expanded, cpu_dijkstra.metrics.goal_cost,
                cpu_dijkstra.metrics.path_dynamic_risk, cpu_dijkstra.metrics.max_dynamic_risk,
                cpu_dijkstra.metrics.reached);
    std::printf("GPU spatiotemporal neural A*: %.3f ms (%d x %dx%d queries, expanded/query %d, %.1f%% fewer than dynamic Dijkstra, dynamic max risk %.3f -> %.3f, %.1f%% risk reduction, %.1fx vs CPU sequential spatiotemporal A*, steps %d, reached %d)\n",
                gpu_ms, BATCH_QUERIES, GRID_W, GRID_H, gpu_metrics.expanded,
                expansion_reduction, cpu_static.metrics.max_dynamic_risk,
                gpu_metrics.max_dynamic_risk, risk_reduction, speedup,
                gpu_metrics.path_steps, gpu_metrics.reached);
    ...
    if (!video.isOpened()) {
        std::fprintf(stderr, "failed to open gif/gpu_spatiotemporal_neural_astar_traversability.avi\n");
        return 1;
    }
    for (const Snapshot& s : snapshots) {
        video.write(draw_frame(display_cells, s, gpu_ms, cpu_dynamic_ms,
                               cpu_static.metrics, cpu_dijkstra.metrics, dijkstra_ms, 0.0f));
    }
    Snapshot final_snap{gpu_metrics.expanded, gpu_metrics.goal_idx, gpu_open, gpu_closed,
                        gpu_path, gpu_metrics};
    for (int i = 0; i < 14; i++) {
        video.write(draw_frame(display_cells, final_snap, gpu_ms, cpu_dynamic_ms,
                               cpu_static.metrics, cpu_dijkstra.metrics, dijkstra_ms, 0.0f));
    }
    video.release();

    avi_to_gif("gif/gpu_spatiotemporal_neural_astar_traversability.avi",
               "gif/gpu_spatiotemporal_neural_astar_traversability.gif", 8, 560);
    std::printf("GIF saved to gif/gpu_spatiotemporal_neural_astar_traversability.gif\n");

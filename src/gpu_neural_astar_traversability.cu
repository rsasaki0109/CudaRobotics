// gpu_neural_astar_traversability.cu
//
// Batched GPU neural A* on learned traversability cost.
//
...
// 192x128 grid, then expands the best node and relaxes its neighbours.  The
// demo compares 64 batched learned-heuristic A* queries against sequential CPU
// neural A* and uninformed Dijkstra expansion on the same cost field.
//
// Output: gif/gpu_neural_astar_traversability.gif
...
static cv::Mat draw_frame(const std::vector<Cell>& cells,
    ...
    double cpu_batch_ms = cpu_ms * static_cast<double>(BATCH_QUERIES);
    double speedup = gpu_ms > 0.0 ? cpu_batch_ms / gpu_ms : 0.0;
    char buf[256];
    std::snprintf(buf, sizeof(buf),
                  "GPU neural A* batch  %dx%dx%d  gpu=%.2f ms  cpu_seq=%.0f ms  %.1fx  expanded -%.1f%%",
                  BATCH_QUERIES, GRID_W, GRID_H, gpu_ms, cpu_batch_ms, speedup, reduction);
    ...
    cv::putText(img, "parallel open-set A* frontier", cv::Point(HALF_W + 14, HEADER_H + 24),
                cv::FONT_HERSHEY_SIMPLEX, 0.52, cv::Scalar(245, 245, 245), 1, cv::LINE_AA);
    std::snprintf(buf, sizeof(buf),
                  "expanded %d / dijkstra %d  path steps=%d  g(goal)=%.2f  reached=%d  dijkstra cpu=%.1f ms",
                  snap.metrics.expanded, dijkstra_metrics.expanded, snap.metrics.path_steps,
                  snap.metrics.goal_cost, snap.metrics.reached, dijkstra_ms);
...
int main() {
    ...
    SearchResult cpu_neural;
    SearchResult cpu_dijkstra;
    double cpu_ms = timed_cpu_search(cells, heuristic, HEURISTIC_WEIGHT, cpu_neural);
    ...
    std::vector<Snapshot> snapshots;
    SearchResult visual = cpu_search(cells, heuristic, HEURISTIC_WEIGHT, &snapshots);
    ...
    std::printf("CPU neural A*: %.3f ms, expanded %d, path cost %.3f, reached %d\n",
                cpu_ms, cpu_neural.metrics.expanded, cpu_neural.metrics.goal_cost,
                cpu_neural.metrics.reached);
    std::printf("GPU batched neural A*: %.3f ms (%d x %dx%d queries, expanded/query %d, %.1f%% fewer than Dijkstra, %.1fx vs CPU sequential neural A*, steps %d, reached %d)\n",
    ...
    if (!video.isOpened()) {
        std::fprintf(stderr, "failed to open gif/gpu_neural_astar_traversability.avi\n");
        return 1;
    }
    for (const Snapshot& s : snapshots) {
        video.write(draw_frame(cells, s, gpu_ms, cpu_ms, cpu_dijkstra.metrics, dijkstra_ms));
    }
    Snapshot final_snap{gpu_metrics.expanded, gpu_metrics.goal_idx, gpu_open, gpu_closed,
                        gpu_path, gpu_metrics};
    for (int i = 0; i < 14; i++) {
        video.write(draw_frame(cells, final_snap, gpu_ms, cpu_ms,
                               cpu_dijkstra.metrics, dijkstra_ms));
    }
    video.release();

    avi_to_gif("gif/gpu_neural_astar_traversability.avi",
               "gif/gpu_neural_astar_traversability.gif", 8, 560);
    std::printf("GIF saved to gif/gpu_neural_astar_traversability.gif\n");

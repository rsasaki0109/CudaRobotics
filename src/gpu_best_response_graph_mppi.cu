// gpu_best_response_graph_mppi.cu
//
// GPU graph-neural best-response MPPI game planner.
//
// A batch of robots crosses a shared interaction zone.  The baseline commits a
// one-shot MPPI plan from intent-belief graph messages.  The game variant then
// treats that committed team trajectory as the other players' strategy profile
// and evaluates one GPU best-response pass for every robot.
//
// Output: gif/gpu_best_response_graph_mppi.gif
...
static cv::Mat draw_frame(const std::vector<RobotSpec>& robots,
    ...
    std::snprintf(buf, sizeof(buf),
                  "GPU best-response graph MPPI  %d robots x %d rollouts x H=%d  gpu=%.2f ms  %.1fx",
    ...
    cv::putText(img, "graph-neural best response", cv::Point(HALF_W + 14, HEADER_H + 24),
                cv::FONT_HERSHEY_SIMPLEX, 0.50, cv::Scalar(245, 245, 245), 1, cv::LINE_AA);
    std::snprintf(buf, sizeof(buf),
    ...
    std::snprintf(buf, sizeof(buf),
                  "unilateral gain %.2f (%.1f%%)   reach %d/%d -> %d/%d   intent top1 %.1f%%",
...
int main() {
    ...
    double cpu_response_ms = evaluate_cpu_rollouts(robots, beliefs, cpu_one_shot_x.data(),
                                                  cpu_one_shot_y.data(), 1, cpu_response);
    double cpu_ms = cpu_one_shot_ms + cpu_response_ms;
    ...
    rollout_kernel<<<blocks, THREADS>>>(d_robots, d_beliefs, nullptr, nullptr, 0,
                                        d_one_shot, d_sample_x, d_sample_y);
    CUDA_CHECK(cudaEventRecord(ev1));
    CUDA_CHECK(cudaEventSynchronize(ev1));
    float gpu_one_shot_ms_f = 0.0f;
    CUDA_CHECK(cudaEventElapsedTime(&gpu_one_shot_ms_f, ev0, ev1));
    ...
    CUDA_CHECK(cudaMemcpy(d_peer_x, one_shot_x.data(), path_total * sizeof(float),
                          cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaMemcpy(d_peer_y, one_shot_y.data(), path_total * sizeof(float),
                          cudaMemcpyHostToDevice));

    CUDA_CHECK(cudaEventRecord(ev0));
    rollout_kernel<<<blocks, THREADS>>>(d_robots, d_beliefs, d_peer_x, d_peer_y, 1,
                                        d_response, d_sample_x, d_sample_y);
    CUDA_CHECK(cudaEventRecord(ev1));
    CUDA_CHECK(cudaEventSynchronize(ev1));
    float gpu_response_ms_f = 0.0f;
    CUDA_CHECK(cudaEventElapsedTime(&gpu_response_ms_f, ev0, ev1));
    CUDA_CHECK(cudaEventDestroy(ev0));
    CUDA_CHECK(cudaEventDestroy(ev1));
    CUDA_CHECK(cudaGetLastError());
    double gpu_ms = static_cast<double>(gpu_one_shot_ms_f + gpu_response_ms_f);
    ...
    CUDA_CHECK(cudaMemcpy(response_results.data(), d_response, total * sizeof(RolloutResult),
                          cudaMemcpyDeviceToHost));
    ...
    reconstruct_paths(robots, beliefs, one_shot_x.data(), one_shot_y.data(), response_selected, 1,
                      response_x, response_y);
    TeamMetrics one_shot = compute_team_metrics(robots, one_shot_results, one_shot_selected,
                                                one_shot_x, one_shot_y);
    TeamMetrics response = compute_team_metrics(robots, response_results, response_selected,
                                                response_x, response_y);
    GameStats game_stats = compute_game_stats(response_results, one_shot_selected,
                                              response_selected);

    double speedup = gpu_ms > 0.0 ? cpu_ms / gpu_ms : 0.0;
    float collision_drop = 100.0f * (1.0f - static_cast<float>(response.collisions)
                                           / static_cast<float>(std::max(one_shot.collisions, 1)));
    float risk_drop = 100.0f * (1.0f - response.mean_social_risk
                                      / std::max(one_shot.mean_social_risk, 1.0e-6f));
    float cvar_drop = 100.0f * (1.0f - response.collision_cvar
                                      / std::max(one_shot.collision_cvar, 1.0e-6f));
    float sep_gain = response.min_separation - one_shot.min_separation;
    ...
    std::printf("CPU one-shot+best-response MPPI: %.3f ms (%d robots x %d rollouts x H=%d x 2 passes; best-response %.3f ms)\n",
                cpu_ms, N_ROBOTS, ROLLOUTS_PER_ROBOT, HORIZON, cpu_response_ms);
    std::printf("GPU best-response graph MPPI: %.3f ms (one-shot+best-response batches, %.1fx vs CPU equivalent rollout eval)\n",
                gpu_ms, speedup);
    ...
    std::printf("Best-response team: collisions %d, reached %d/%d, deadlocks %d, min sep %.3f, terminal avg %.3f, social risk avg/max %.3f/%.3f, collision CVaR %.3f, unilateral gain %.3f (%.2f%%, max %.3f), collision reduction %.1f%%, risk reduction %.1f%%, CVaR reduction %.1f%%, separation gain %.3f\n",
                response.collisions, response.reached, N_ROBOTS, response.deadlocks,
                response.min_separation, response.mean_terminal,
                response.mean_social_risk, response.max_social_risk, response.collision_cvar,
                game_stats.mean_unilateral_gain, game_stats.normalized_gain,
    ...
    if (!video.isOpened()) {
        std::fprintf(stderr, "failed to open gif/gpu_best_response_graph_mppi.avi\n");
        return 1;
    }
    for (int k = 2; k <= HORIZON; k += 2) {
        video.write(draw_frame(robots, one_shot_x, one_shot_y, response_x, response_y,
                               sample_x, sample_y, one_shot, response, intent_stats,
                               game_stats, gpu_ms, cpu_ms, k));
    }
    for (int i = 0; i < 12; i++) {
        video.write(draw_frame(robots, one_shot_x, one_shot_y, response_x, response_y,
                               sample_x, sample_y, one_shot, response, intent_stats,
                               game_stats, gpu_ms, cpu_ms, HORIZON));
    }
    video.release();

    avi_to_gif("gif/gpu_best_response_graph_mppi.avi",
               "gif/gpu_best_response_graph_mppi.gif", 8, 560);
    std::printf("GIF saved to gif/gpu_best_response_graph_mppi.gif\n");
